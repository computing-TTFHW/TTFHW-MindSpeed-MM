...
def mel_filters(device, n_mels: int) -> torch.Tensor:
    ...
    if n_mels not in {80, 128}:
        raise AssertionError(f"Unsupported n_mels: {n_mels}")
...
def sinusoids(length, channels, max_timescale=10000):
    ...
    if channels % 2 != 0:
        raise AssertionError(f"channels should be even, but got {channels}")
...
class MultiHeadAttention(nn.Module):
    ...
    def qkv_attention_manual(
        ...
        if output_packed.shape != (n_ctx, n_state):
            raise AssertionError(f"output packed shape should be ({n_ctx}, {n_state}), but got {output_packed.shape}")
